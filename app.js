...
(function () {
  ...
  function toggleSelect(frame, entry) {
  ...
  function makeCard(opts) {
    ...
    entry.setFrame = function (frame) {
      ...
      if (checkbox) {
        ...
        checkbox.addEventListener("change", () => toggleSelect(frame, entry));
      }
  ...
  // Fill a list of card entries with frames sequentially (cancellable).
  ...
  function renderFocus(centerTime) {
    ...
    centerEntry.setFrame = function (frame) {
      ...
      heroCb.addEventListener("change", () => toggleSelect(frame, centerEntry));
  ...
  function renderFree() {
    ...
    heroMedia.addEventListener("click", () => {
      ...
      navTo({ type: "rangeDetail", start: start, end: Math.max(end, start + 0.5) });
    });
  ...
  function renderRangeDetail(rangeStart, rangeEnd) {
    ...
    const fStart = rangeField("Начало", start, "range--green");
    const fEnd = rangeField("Конец", end, "range--pink");
    panel.appendChild(fStart.field);
    panel.appendChild(fEnd.field);
    ...
    fStart.slider.max = state.duration.toFixed(2);
    fEnd.slider.max = state.duration.toFixed(2);
    ...
    fStart.slider.addEventListener("input", () => {
      ...
      if (b - a < 0.5) { b = clamp(a + 0.5, 0, state.duration); fEnd.slider.value = b.toFixed(2); fEnd.val.textContent = fmtTime(b); }
    ...
    fEnd.slider.addEventListener("input", () => {
      let a = parseFloat(fStart.slider.value);
      let b = parseFloat(fEnd.slider.value);
      if (b - a < 0.5) { a = clamp(b - 0.5, 0, state.duration); fStart.slider.value = a.toFixed(2); fStart.val.textContent = fmtTime(a); }
    ...
    function build(a, b) {
      ...
      // update head chip
      const chip = contentEl.querySelector(".section-head .chip") || (function () {
        const c = document.createElement("span"); c.className = "chip";
        contentEl.querySelector(".section-head").appendChild(c); return c;
      })();
  ...
  function rangeField(label, value, cls) {
    const field = document.createElement("div");
    field.className = "field";
    ...
    slider.step = "0.1"; slider.value = value.toFixed(2);
